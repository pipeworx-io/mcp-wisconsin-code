 ...
 *
 * Built, dropped, and rebuilt. The first attempt anchored on the section
 * number at a line start, the way the Missouri and Minnesota packs do, and it
 * failed here in a way worth recording: Wisconsin heads its History and Note
 * blocks with the SAME section number and places them BEFORE the statute, so
 * every positional anchor landed on editorial apparatus. A caller would have
 * read "940.01 HistoryHistory: 1987 a. 399" where the law belongs.
 *
 * Position is the wrong signal here; the markup carries an explicit one. Each
 * subsection is tagged with the citation it belongs to:
 *
 *   <span data-cites='["statutes/940.01(1)(a)"]'> ... <span class="qstr">text</span>
 *
 * So the statute is assembled from the qstr spans whose data-cites names THIS
 * section, and the annotations — which cite the section but are not part of it
 * — never enter the text because they are not inside a cited subsection span.
 *
 * The lesson generalises: when a page tells you what each fragment IS, use
 * that, rather than inferring meaning from where the fragment sits.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wisconsin Statutes');
}

const BASE = 'https://docs.legis.wisconsin.gov/document/statutes/';
const UA = 'pipeworx-mcp-wisconsin-code/1.0 (+https://pipeworx.io)';
...
const tools: McpToolExport['tools'] = [{
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "940.01" (first-degree intentional homicide) or "346.63" (OWI).' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'wi_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^s\.?/i, '');
  if (!/^\d+\.\d+$/.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Wisconsin Statutes citation.`,
      hint: 'Wisconsin sections look like "940.01" (homicide) or "346.63" (OWI).' };
  }
  ...
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the Wisconsin Statutes (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Wisconsin Legislature returned HTTP ${res.status} for ${raw}`);
  const html = await res.text();

  // Assemble from subsection spans that name this section, in document order.
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const cited = new RegExp(`data-cites='\\["statutes/${esc}[^\\]]*"\\]'([\\s\\S]{0,4000}?)(?=data-cites=|$)`, 'g');
  const parts: string[] = [];
  let m: RegExpExecArray | null;
  while ((m = cited.exec(html)) !== null) {
    const chunk = m[1]
      .replace(/<a\b[^>]*>[\s\S]*?<\/a>/gi, (a) => a.replace(/<[^>]+>/g, ''))
      .match(/<span class="qstr">([\s\S]*?)<\/span>/g);
    if (chunk) parts.push(chunk.map((c) => statuteText(c)).join(' '));
  }
  const body = statuteText(parts.join('\n')).replace(/\n{2,}/g, '\n').trim();

  if (body.length < 40) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No text found for Wisconsin Statute ${raw}. The page exists but carries no subsection tagged to this section, which usually means it was repealed or renumbered.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const heading = (statuteText(html.match(/<h[12][^>]*>([\s\S]{0,200}?)<\/h[12]>/i)?.[1] ?? '') || null);

  return {
    citation: `Wis. Stat. § ${raw}`, section: raw, heading,
    text: body, text_chars: body.length, url,
    source: 'Wisconsin Legislature (docs.legis.wisconsin.gov), keyless',
    attribution: 'Wisconsin statutes are public record.',
  };
}
